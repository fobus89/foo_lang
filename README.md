...
2. **Нет работы с файловой системой** (чтение/запись файлов)
3. **Нет поддержки JSON** (parse/stringify)
4. **Нет регулярных выражений**
5. **Нет поддержки многопоточности**
6. **Нет пакетного менеджера зависимостей**
7. **Строковая интерополяция** не поддерживает вложенные строки в выражениях
